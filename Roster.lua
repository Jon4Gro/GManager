...
-- and pop our own panel that shows everything GManager tracks plus the
...
local function buildPanel()
    ...
    local f = CreateFrame("Frame", "GManagerDetailPanel", UIParent)
 ...
 --   f.officerEdit:SetSize(250, 20)
    ...
    f.btnRemove:SetScript("OnClick", function()
        if not currentName then return end
        StaticPopupDialogs["GManager_CONFIRM_REMOVE"] = {
        ...
        StaticPopup_Show("GManager_CONFIRM_REMOVE")
    end)
...
function addon:ShowMemberDetail(name)
    ...
    if not panel:IsShown() then
        ...
        local mainFrame = _G["GManagerMainFrame"]
...
local function attach()
    ...
    if GuildMemberDetailFrame.__GManagerTakeover then return end
    GuildMemberDetailFrame.__GManagerTakeover = true
    ...
    panel:HookScript("OnUpdate", function(self, elapsed)
        self.__GManagerTimer = (self.__GManagerTimer or 0) + (elapsed or 0)
        if self.__GManagerTimer < 0.1 then return end
        self.__GManagerTimer = 0
...
retry:SetScript("OnEvent", function(self)
    if GuildMemberDetailFrame then
        attach()
        if GuildMemberDetailFrame.__GManagerTakeover then
